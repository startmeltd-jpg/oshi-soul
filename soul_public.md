...
**環境変数の事前確認**: `/home/ubuntu/oshi-core/.env` が存在することを確認してから実行する。
...
1. MapReduce的な並列処理の禁止: wide_researchやmap関数でサブタスクを大量に並列作成することを禁止する
2. LLMレビューの順次実行: 複数のLLMでレビューする場合は1つずつ順番に実行する
...
記憶の読み込みフロー:
1. `.env` の存在確認 → なければ作成
2. `cd /home/ubuntu/oshi-core && python3 CORE/scripts/session_startup.py --inject`
3. フォールバック → `cd /home/ubuntu/oshi-core && python3 CORE/scripts/memory_loader.py critical`
4. 最終手段 → `memory/handoff_latest.json` を直接読む
